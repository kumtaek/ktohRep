...
# -*- coding: utf-8 -*-
"""
ERD JSON 데이터를 사용하여 HTML 리포트를 생성하는 스크립트
"""
...
<body>
    ...
    <script>
        ...
        // 노드 추가
        Object.values(erdData.nodes).forEach(node => {
            cy.add({
                group: 'nodes',
                data: {
                    id: node.id,
...
def main():
    """메인 실행 함수"""
    print("=" * 80)
    print("🚀 ERD HTML 리포트 생성")
    print("=" * 80)
    
    # 새로운 폴더 구조에 맞춰 JSON 파일 찾기
    json_dir = Path("./project/sampleSrc/report")
    json_files = list(json_dir.glob("erd_data_*.json"))
    
    if not json_files:
        print("❌ ERD JSON 파일을 찾을 수 없습니다.")
    ...
    html_path = json_dir / f"erd_{timestamp}.html"
