...
class SecurityVulnerabilityDetector:
    ...
    def detect_sql_injection_patterns(self, content: str, file_type: str, file_path: str) -> List[Dict]:
        """SQL Injection 패턴 탐지"""
        vulnerabilities = []
        ...
        if file_type == 'jsp':
            ...
            patterns = [
                ...
                (r'\.append\s*\(\s*["\'][^"\']*["\'].*?\)\.append\s*\([^)]*\)', 'HIGH', 
                 'Dynamic SQL construction with StringBuilder vulnerable to injection'),
                # 직접 문자열 결합
                (r'["\'][^"\']*(?:SELECT|INSERT|UPDATE|DELETE)[^"\']*["\'].*?\+.*?[^"\']+', 'HIGH',
                 'String concatenation for SQL construction vulnerable to injection'),
                # JSP 표현식 내 SQL
                (r'<%.*?["\'][^"\']*(?:SELECT|INSERT|UPDATE|DELETE)[^"\']*["\'].*?%>', 'MEDIUM',
                 'JSP scriptlet contains dynamic SQL construction')
            ]
            ...
            for pattern, severity, message in patterns:
                ...
                for match in matches:
                    ...
                    vulnerabilities.append({
                        ...
                        'type': 'SQL_INJECTION',
                        'severity': severity,
                        'pattern': match.group()[:200],  # 200자로 제한
                        'message': message,
            ...
            unsafe_patterns = list(re.finditer(r'\$\{[^}]+\}', content))
            ...
            # 직접 SQL 삽입 패턴
            direct_sql_patterns = list(re.finditer(r'<select[^>]*>\s*\$\{sql\}\s*</select>', content, re.IGNORECASE))
            for match in direct_sql_patterns:
                line_number = content[:match.start()].count('\n') + 1
                vulnerabilities.append({
                    'file_path': file_path,
                    'type': 'SQL_INJECTION',
                    'severity': 'CRITICAL',
                    'pattern': match.group(),
                    'message': 'Direct SQL injection through ${sql} parameter',
                    'line_number': line_number,
                    'confidence': 0.99
                })
...
class ImprovedJspMybatisParser:
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        self.vulnerability_detector = SecurityVulnerabilityDetector()
    ...
    def parse_file(self, file_path: str, project_id: int) -> Tuple[File, List[SqlUnit], List[Join], List[RequiredFilter], List[Edge], List[Dict]]:
        ...
        try:
            ...
            # 보안 취약점 탐지
            if file_path.endswith('.jsp'):
                vuln = self.vulnerability_detector.detect_sql_injection_patterns(content, 'jsp', file_path)
                vulnerabilities.extend(vuln)
            elif file_path.endswith('.xml'):
                vuln = self.vulnerability_detector.detect_sql_injection_patterns(content, 'mybatis', file_path)
                vulnerabilities.extend(vuln)
