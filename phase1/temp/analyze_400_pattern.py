...
def analyze_400_pattern():
    ...
    pattern = r'public\s+\w+\s+\w+\s*\([^)]*\)\s*\{[^}]{,400}\}'
    
    print("=== 400자 이하 패턴 분석 결과 ===")
    print()
    
    non_matching_under_400 = []
    non_matching_over_400 = []
    ...
    for name, test_case, expected_length in test_cases:
        ...
        if match:
            ...
            result = "매치안됨"
            if expected_length <= 400:
                non_matching_under_400.append((name, expected_length))
            else:
                non_matching_over_400.append((name, expected_length))
        
        print(f"{name}: {expected_length}자 → {result}")
    
    print()
    print("=== 문제 케이스 (400자 이하인데 매치 안됨) ===")
    for name, length in non_matching_under_400:
        print(f"- {name}: {length}자")
    
    print()
    print("=== 400자 초과 케이스 (매치 안되는 것이 올바름) ===")
    for name, length in non_matching_over_400:
        print(f"- {name}: {length}자")
    
    if non_matching_over_400:
        max_length = max(length for _, length in non_matching_over_400)
