...
"""
ERD 생성 기능 테스트 스크립트
기존 메타데이터를 사용하여 ERD가 제대로 생성되는지 테스트합니다.
"""
...
visualize_path = current_dir / "visualize"
sys.path.insert(0, str(visualize_path))
...
def test_erd_generation():
    ...
    try:
        ...
        # 설정 파일 로드 (루트 config 폴더 사용)
        config_path = current_dir / "config" / "visualize" / "config.yaml"
        if not config_path.exists():
            # 루트 config 폴더 확인
            root_config_path = current_dir / "config" / "config.yaml"
            if root_config_path.exists():
                config_path = root_config_path
                print(f"✅ 루트 설정 파일 사용: {config_path}")
            else:
