...
class SimpleImprovedJavaParser:
    """간단한 개선된 Java 파서 - 과추출 문제 해결"""
...
def test_simple_improved_parser():
    """간단한 개선된 파서 테스트"""
    
    print("=== 간단한 개선된 Java 파서 테스트 ===\n")
    ...
    # 개선된 파서로 분석
    parser = SimpleImprovedJavaParser()
    ...
    # 이전 결과와 비교
    print(f"\n=== 이전 결과와 비교 ===")
    print(f"이전 복잡한 패턴: 51개 메소드 (750% 과추출)")
    print(f"개선된 패턴: {len(result['methods'])}개 메소드")
    if len(result['methods']) > 0:
        improvement = ((51 - len(result['methods'])) / 51 * 100)
        print(f"개선률: {improvement:.1f}%")

if __name__ == "__main__":
    test_simple_improved_parser()
