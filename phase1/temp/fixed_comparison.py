...
def get_metadb_results():
    ...
    try:
        ...
        print(f"메타디비 전체 메소드 수: {total_methods}개")
        
        # 전체 SQL Units 수 확인
        cursor.execute("SELECT COUNT(*) FROM sql_units")
        total_sql_units = cursor.fetchone()[0]
        print(f"메타디비 전체 SQL Units 수: {total_sql_units}개")
...
def generate_detailed_comparison():
    ...
    print(f"메타디비 전체 메소드: {metadb_results['total_methods']}개")
    print(f"메타디비 전체 SQL Units: {metadb_results['total_sql_units']}개")
    print()
