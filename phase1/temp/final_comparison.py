...
def get_manual_analysis_results():
    ...
    for java_file in java_files:
        ...
        try:
            ...
            java_results[abs_path] = total_methods
            
        except Exception as e:
            java_results[abs_path] = 0
    ...
    for xml_file in xml_files:
        ...
        try:
            ...
            xml_results[abs_path] = total_queries
            
        except Exception as e:
            xml_results[abs_path] = 0
    ...
    for jsp_file in jsp_files:
        abs_path = os.path.abspath(jsp_file)
        try:
            ...
            jsp_results[abs_path] = total_scripts
            
        except Exception as e:
            jsp_results[abs_path] = 0
...
def generate_final_comparison():
    """최종 상세 비교 리포트"""
    print("=== 수동 분석 vs 메타디비 상세 비교 (절대 경로 기준) ===\n")
    ...
    print(f"{'파일명':<80} {'수동':<8} {'메타디비':<10} {'차이':<8} {'정확도':<8}")
    print("-" * 120)
    ...
    for file_path, manual_count in manual_results['java'].items():
        ...
        # 파일명만 표시 (절대 경로가 너무 길어서)
        filename = os.path.basename(file_path)
        print(f"{filename:<80} {manual_count:<8} {metadb_count:<10} {diff:<8} {accuracy:.1f}%")
        
        java_total_manual += manual_count
        java_total_metadb += metadb_count
    
    print("-" * 120)
    print(f"{'Java 총계':<80} {java_total_manual:<8} {java_total_metadb:<10} {java_total_manual - java_total_metadb:<8} {(java_total_metadb / java_total_manual * 100):.1f}%")
    print()
    ...
    print("-" * 120)
    print(f"{'XML 총계':<80} {xml_total_manual:<8} {xml_total_metadb:<10} {xml_total_manual - xml_total_metadb:<8} {(xml_total_metadb / xml_total_manual * 100):.1f}%")
    print()
    ...
    print("-" * 120)
    print(f"{'JSP 총계':<80} {jsp_total_manual:<8} {jsp_total_metadb:<10} {jsp_total_manual - jsp_total_metadb:<8} {(jsp_total_metadb / jsp_total_manual * 100):.1f}%")
    print()
