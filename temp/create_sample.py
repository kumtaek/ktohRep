...
def create_sample():
    ...
    # 디렉토리 생성
    os.makedirs('../project/sampleSrc/src/main/java/com/example/controller', exist_ok=True)
    os.makedirs('../project/sampleSrc/src/main/java/com/example/model', exist_ok=True)
    os.makedirs('../project/sampleSrc/src/main/webapp/user', exist_ok=True)
    os.makedirs('../project/sampleSrc/db_schema', exist_ok=True)
    
    # Java 파일 생성
    java_content = '''package com.example.controller;
...
<body>
    ...
    <div class="search-form">
        <form method="get" action="<c:url value='/user/list'/>">
            ...
            <a href="<c:url value='/user/list'/>">초기화</a>
        </form>
    </div>
    ...
    # SQL 파일 생성
    sql_content = '''-- Oracle DDL 스크립트 (다이나믹 쿼리용)
CREATE TABLE users (
...
COMMIT;'''
    ...
    with open('../project/sampleSrc/db_schema/oracle_schema.sql', 'w', encoding='utf-8') as f:
        f.write(sql_content)
