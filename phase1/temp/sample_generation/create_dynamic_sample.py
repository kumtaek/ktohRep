...
"""
다이나믹 쿼리가 포함된 JSP, Java, MyBatis 샘플 생성
"""
...
def backup_existing_sample():
    ...
    backup_dir = f'../project/sampleSrc_backup_{datetime.now().strftime("%Y%m%d_%H%M%S")}'
    
    if os.path.exists(source_dir):
        shutil.copytree(source_dir, backup_dir)
        print(f"✅ 기존 샘플 백업: {backup_dir}")
...
def create_sample_structure():
    ...
    print("✅ 샘플 디렉토리 구조 생성 완료")

def create_java_files():
    """Java 파일들 생성 (다이나믹 쿼리 포함)"""
...
public interface UserService {
    ...
    with open('../project/sampleSrc/src/main/java/com/example/service/UserService.java', 'w', encoding='utf-8') as f:
        f.write(user_service)
    
    # UserServiceImpl.java
    user_service_impl = '''package com.example.service;
...
public class UserServiceImpl implements UserService {
    ...
    with open('../project/sampleSrc/src/main/java/com/example/service/UserServiceImpl.java', 'w', encoding='utf-8') as f:
        f.write(user_service_impl)
    
    # UserMapper.java
    user_mapper = '''package com.example.mapper;
...
public interface UserMapper {
    ...
    with open('../project/sampleSrc/src/main/java/com/example/mapper/UserMapper.java', 'w', encoding='utf-8') as f:
        f.write(user_mapper)
...
<mapper namespace="com.example.mapper.UserMapper">
    ...
    with open('../project/sampleSrc/src/main/resources/mybatis/mapper/UserMapper.xml', 'w', encoding='utf-8') as f:
        f.write(user_mapper_xml)
    
    print("✅ MyBatis XML 파일들 생성 완료")

def create_jsp_files():
    """JSP 파일들 생성 (다이나믹 쿼리 포함)"""
...
<body>
    ...
    with open('../project/sampleSrc/src/main/webapp/user/list.jsp', 'w', encoding='utf-8') as f:
        os.makedirs('../project/sampleSrc/src/main/webapp/user', exist_ok=True)
        f.write(user_list_jsp)
    
    # user/searchResult.jsp
    search_result_jsp = '''<%@ page language="java" contentType="text/html; charset=UTF-8" pageEncoding="UTF-8"%>
<%@ taglib prefix="c" uri="http://java.sun.com/jsp/jstl/core" %>
<%@ taglib prefix="fmt" uri="http://java.sun.com/jsp/jstl/fmt" %>
<!DOCTYPE html>
<html>
...
<body>
    ...
    <div class="search-info">
        ...
        <c:if test="${not empty searchParams.userType}">
            <p>사용자 타입: ${searchParams.userType}</p>
        </c:if>
    ...
    <a href="<c:url value='/user/list'/>" class="btn">목록으로</a>
</body>
</html>'''

    with open('../project/sampleSrc/src/main/webapp/user/searchResult.jsp', 'w', encoding='utf-8') as f:
        f.write(search_result_jsp)
