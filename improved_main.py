    ...
    async def analyze_project(self, project_root: str, project_name: str = None) -> Dict[str, Any]:
        ...
        with PerformanceLogger(self.logger, f"프로젝트 '{project_name}' 전체 분석"):
            ...
            try:
                ...
                # 4. 파일들 병렬 분석 (개선된 병렬 처리)
                self.logger.info("병렬 파일 분석 시작")
                analysis_results = await self.metadata_engine.analyze_files_parallel(
                    source_files, project_id, self.parsers
                )
                ...
                return {
                    ...
                    'files_processed': analysis_results['total_files'],
                    'files_success': analysis_results['success_count'],
                    'files_error': analysis_results['error_count'],
...
def main():
    ...
    parser = argparse.ArgumentParser(
        ...
        epilog="""
예시 사용법:
  python improved_main.py PROJECT/sample-app
  python improved_main.py PROJECT/sample-app --project-name "샘플 앱"
  python improved_main.py PROJECT/sample-app --config custom_config.yaml
        """
    )
    ...
    try:
        ...
        analyzer = ImprovedSourceAnalyzer(args.config)
...
def print_analysis_results(result: Dict[str, Any]):
    ...
    print("파일 처리 현황:")
    print(f"  - 전체 파일: {results['total_files']}개")
    ...
    # 성능 요약
    if results['total_files'] > 0 and result['analysis_time'] > 0:
        files_per_sec = results['total_files'] / result['analysis_time']
