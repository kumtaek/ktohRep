...
def build_relatedness_graph_json(
    ...
    for rel in relatedness_pairs[:max_nodes * 2]:  # 엣지 수 제한으로 노드 수 간접 제어
        ...
        if src_key not in nodes:
            details = _get_node_details(db, rel.node1_type, rel.node1_id)
            label = _get_node_label(rel.node1_type, details)
            nodes[src_key] = create_node(
                src_key, rel.node1_type, label, "unclustered", details
            )

        # 대상 노드 추가
        if dst_key not in nodes:
            details = _get_node_details(db, rel.node2_type, rel.node2_id) 
            label = _get_node_label(rel.node2_type, details)
            nodes[dst_key] = create_node(
                dst_key, rel.node2_type, label, "unclustered", details
            )
