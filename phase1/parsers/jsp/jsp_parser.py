...
class JSPParser(JSPParserContext7):
    ...
    def parse_content(self, content: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        # JSP 특화 검증
        jsp_errors = self._validate_jsp_syntax(normalized_content)
        el_errors = self._validate_el_expressions(normalized_content)
        tag_errors = self._validate_tag_matching(normalized_content)
        ...
        result = {
            ...
            'parsing_quality': self._calculate_jsp_quality(normalized_content, jsp_errors + el_errors + tag_errors),
            'confidence': 0.9
        }
        
        return result
    
    def _validate_jsp_syntax(self, content: str) -> List[Dict[str, Any]]:
        """JSP 구문 검증"""
        jsp_errors = []
        
        # 기본적인 JSP 구문 검증
        # 예: JSP 태그 닫기, 지시자 문법 등
    ...
    def _is_in_foreach_scope(self, content: str, expression: str) -> bool:
        """EL 표현식이 forEach 스코프 내부에 있는지 확인"""
        # 간단한 휴리스틱: forEach 태그와의 위치 관계 확인
        el_pos = content.find(f'${{{expression}}}')
        if el_pos == -1:
            return False
        
        # 앞쪽에서 가장 가까운 forEach 태그 찾기
        before_content = content[:el_pos]
        foreach_pos = before_content.rfind('<c:forEach')
        
        if foreach_pos == -1:
            return False
        
        # forEach 태그가 닫혔는지 확인
        after_foreach = content[foreach_pos:]
        endforeach_pos = after_foreach.find('</c:forEach>')
        
        if endforeach_pos == -1:
            return True  # 닫히지 않은 forEach 내부
        
        # EL 표현식이 forEach 범위 내부에 있는지 확인
        return el_pos < foreach_pos + endforeach_pos
    
    def _calculate_jsp_quality(self, content: str, jsp_errors: List) -> float:
        """JSP 파싱 품질 점수 계산"""
        base_score = 1.0
        error_penalty = len(jsp_errors) * 0.2
