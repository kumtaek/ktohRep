...
class MetaDBValidator:
    def __init__(self, project_name: str):
        ...
        self.manual_report_path = "../Dev.Report/파일별 메소드, 쿼리 개수 조사 결과.md"
    ...
    def load_metadb_analysis(self) -> Dict[str, Any]:
        ...
        try:
            ...
            # 파일별 메소드 개수 조회
            cursor.execute("""
                SELECT f.file_path, COUNT(s.id) as method_count
                FROM files f
            ...
            # 파일별 쿼리 개수 조회
            cursor.execute("""
                SELECT f.file_path, COUNT(s.id) as query_count
                FROM files f
                LEFT JOIN sql_units s ON f.id = s.file_id
                WHERE s.sql_type IS NOT NULL
                GROUP BY f.id, f.file_path
            """)
            ...
            cursor.execute("SELECT COUNT(*) FROM sql_units WHERE file_id IN (SELECT id FROM files WHERE file_type = 'java')")
            total_methods = cursor.fetchone()[0]
            
            cursor.execute("SELECT COUNT(*) FROM sql_units WHERE sql_type IS NOT NULL")
