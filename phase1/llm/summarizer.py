...
class CodeSummarizer:
    ...
    def _chat_with_debug(self, client, messages: list, **kwargs):
        ...
        korean_system = {
            "role": "system",
            "content": "모든 응답을 한국어로 해주세요. 정중하고 공식적인 말투로 간결하고 명확하고 중복되지 않도록 답변해주세요.\n" + \
                       "딱 필요한 답변만 하고 네, 알겠습니다 같은 군더더기 말은 하지 마세요.\n" + \
                       "근거 없이 추정해서 답하면 안됩니다. 충분한 근거가 있는 답변만 합니다. 정보가 부족하거나, 이해가 안되거나, 답변이 불가한 경우 '-'를 답변주세요.\n" + \
                       "군더더기 없이 할말만 하고, 잘 알지 못하는 내용이나 자신이 없는 답변은 굳이 하지 않습니다.\n" + \
                       "'정보가 부족하므로 상세한 설명은 제공할 수 없습니다' -> 이런 불필요한 말은 하지 않습니다.\n"
        }
    ...
    def _read_project_file_content(self, file_path: str, max_size: int = 10000) -> str:
        ...
        if Path(file_path).is_absolute() :
            #file_path.startswith('PROJECT\\') or file_path.startswith('PROJECT/'):
        ...
        try:
            with open(full_path, 'r', encoding='utf-8', errors='ignore') as f:
                ...
                if len(content) == max_size:
                    logger.info(f"파일 내용이 {max_size}로 제한됨: {full_path}")
                return content
...
이 정보를 바탕으로 파일 전체의 목적과 역할을 2-1~2문장으로 종합 요약해주세요:"""
...
요약:"""
                ...
                prompt = f"""다음 MyBatis {query_type} 쿼리의 목적을 1~2문장 이내로 요약해주세요:
...
요약:"""
                ...
                prompt = f"""다음 Java 클래스의 역할을 1~2문장 이내로 요약해주세요:
...
요약:"""
                ...
                prompt = f"""다음 JSP 스크립틀릿의 기능을 1~2문장 이내로 요약해주세요:
...
요약:"""
                ...
                prompt = f"""다음 코드 블록의 목적을 1~2문장 이내로 요약해주세요:
...
요약:"""
            ...
            summary = self._chat_with_debug(client, [{"role": "user", "content": prompt}], max_tokens=1000, temperature=0.3)
    ...
    def _fallback_file_summary(self, file: File, file_content: str) -> Optional[str]:
        ...
        try:
            ...
            prompt = f"""파일을 분석하고 목적과 기능을 1~2문장 이내로 요약해주세요:

파일: {file.path}
내용:
{file_content[:3000]}

이 파일의 주요 목적과 기능을 1~2문장 이내로 요약해주세요:"""

            summary = self._chat_with_debug(client, [{"role": "user", "content": prompt}], max_tokens=1500, temperature=0.3)
            return summary.strip() if isinstance(summary, str) else str(summary).strip()
    ...
    def summarize_method(self, method: Method) -> Optional[str]:
        ...
        try:
            ...
            logger.debug(f"메서드 {method.name}: method_code = '{method_code[:100]}'")

            # 만약 코드가 없고 파일 경로가 있다면 프로젝트 폴더에서 읽기 시도
            if not method_code and hasattr(method, 'file') and method.file and method.file.path:
                logger.debug(f"파일에서 메서드 코드 읽기 시도: {method.file.path}")
                file_content = self._read_project_file_content(method.file.path)
                logger.debug(f"파일 내용 읽기 결과: {len(file_content) if file_content else 0}자")
                
                # 메소드 이름으로 코드 추출 시도 (start_line, end_line 활용)
                if file_content and method.name:
                    lines = file_content.split('\n')
                    
                    # start_line, end_line이 있으면 우선 사용
                    if hasattr(method, 'start_line') and hasattr(method, 'end_line') and method.start_line and method.end_line:
                        logger.debug(f"start_line: {method.start_line}, end_line: {method.end_line}")
                        if method.start_line <= len(lines) and method.end_line <= len(lines):
                            method_lines = lines[method.start_line-1:method.end_line]
                            method_code = '\n'.join(method_lines)
                            logger.debug(f"start_line/end_line으로 추출: {len(method_code)}자")
                    
                    # start_line/end_line이 없으면 휴리스틱 사용
                    if not method_code:
                        method_start = -1
                        ...
                        if method_start >= 0:
                            max_len_method = 1000
                            logger.debug(f"휴리스틱으로 메서드 시작 찾음: {method_start}행")
                            method_lines = lines[method_start:method_start + max_len_method]
                            method_code = '\n'.join(method_lines)
                            logger.debug(f"휴리스틱으로 추출: {len(method_code)}자")

            # 메서드 코드가 없으면 건너뛰기
            if not method_code:
                logger.warning(f"메서드 코드를 찾을 수 없음: {method.name}")
                return None

            prompt = f"""
다음 Java 메서드를 분석하고 기능을 1~2문장 이내로 요약해주세요.
...
이 메서드가 하는 일과 목적에 대해 1~2문장 이내로 요약 제공해주세요.

요약:

혹시 리팩토링 제안할 내용이 있으면 1~2문장 이내로 요약 제공해주세요.
제안할게 없으면 굳이 억지로 제안을 하지 않아도 됩니다. 할말 없으면 '-'로 답변.
...
개선제안:"""
    ...
    def summarize_sql_unit(self, sql_unit: SqlUnit) -> Optional[str]:
        ...
        try:
            ...
            # 파일 경로가 있다면 프로젝트 폴더에서 읽기 시도
            if hasattr(sql_unit, 'file') and sql_unit.file and sql_unit.file.path:
                ...
                if file_content:
                    ...
                    if sql_unit.stmt_id:
                        ...
                        if match:
                            sql_content = match.group(1).strip()[:max_sql_len]
                    
                    # JSP 파일에서 SQL 추출 시도 (start_line, end_line 활용)
                    if not sql_content and sql_unit.start_line and sql_unit.end_line:
                        file_lines = file_content.split('\n')
                        if sql_unit.start_line <= len(file_lines) and sql_unit.end_line <= len(file_lines):
                            sql_lines = file_lines[sql_unit.start_line-1:sql_unit.end_line]
                            sql_content = '\n'.join(sql_lines)[:max_sql_len]  

            # SQL 내용이 없으면 건너뛰기 (해시값은 사용 안함)
            if not sql_content:
                logger.warning(f"SQL 내용을 찾을 수 없음: {sql_unit.mapper_ns}.{sql_unit.stmt_id}")
                return None

            prompt = f"""
다음 SQL 쿼리/문을 분석하고 기능을 1~2문장 이내로 요약해주세요.
...
SQL 내용:
{sql_content[:max_sql_len]}...

이 SQL 문이 하는 일과 목적에 대해 1~2문장 이내로 요약 제공해주세요.
...
요약:"""
    ...
    def enhance_table_comment(self, table: DbTable, related_code_context: str = "") -> Optional[str]:
        ...
        try:
            ...
            prompt = f"""데이터베이스 테이블을 종합 분석하여 한글로 한문장으로 설명을 제공해주세요.
...
위 모든 정보를 종합하여 테이블에 대한 설명을 한 문장으로 간략히 작성해주세요:

※ 굳이 할말이 없으면 ' '을 답변해도 됩니다.

설명:"""

            comment = self._chat_with_debug(client, [{"role": "user", "content": prompt}], max_tokens=2500, temperature=0.3)
    ...
    def _collect_table_related_chunks(self, table_name: str) -> str:
        ...
        try:
            ...
            session.close()
            return '\n'.join(related_chunks) if related_chunks else f"{table_name}"
    ...
    def enhance_column_comment(self, column: DbColumn, table_context: str = "") -> Optional[str]:
        ...
        try:
            ...
            max_len_code = 10000
            print(f"max_len_code = {max_len_code}")
            prompt = f"""데이터베이스 컬럼을 종합 분석하여 한글로 한문장으로 설명을 제공해주세요.
...
관련 코드 분석 (청킹 기반):
{column_chunk_context[:max_len_code]}

기존 테이블 컨텍스트:
{table_context[:800]}

위 모든 정보를 종합하여 컬럼을 표현하는 한문장으로 요약 작성해주세요.
...
설명:"""
    ...
    def _collect_column_related_chunks(self, column_name: str, table_name: str) -> str:
        ...
        try:
            ...
            session.close()
            return '\n'.join(related_chunks) if related_chunks else f"{column_name}"
    ...
    def analyze_primary_key_candidates(self, table: DbTable, table_context: str = "") -> List[str]:
        ...
        try:
            ...
            try:
                ...
                max_len_context = 10000
                print(f"max_len_context = {max_len_context}")
...
Primary Key로 적합한 컬럼명만 쉼표로 구분하여 나열해주세요. 없으면 '-'이라고 답변해주세요.

추천 Primary Key:"""

                response = self._chat_with_debug(client, [{"role": "user", "content": prompt}], max_tokens=1000, temperature=0.3)
    ...
    def process_project_summaries(self, project_id: int, batch_size: int = 10):
        ...
        try:
            ...
            handle_critical_error(logger, "프로젝트 요약 처리 실패", e)
        finally:
            session.close()
    ...
    def analyze_joins_from_sql(self, sql_unit: SqlUnit) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            try:
                ...
                max_len_sql = 10000
                print(f"max_len_sql = {max_len_sql}")
                if chunk_query and chunk_query[0]:
                    sql_text = chunk_query[0][:max_len_sql]  # 5KB 한도
                else:
...
* JOIN 사례 (아래는 샘플입니다)
    ...
    JOIN customers c ON o.customer_id = c.customer_id and o.cust_name = c.cust_name
    또는 
    o.customer_id = c.customer_id and o.cust_name = c.cust_name
            
    쿼리에 위와 같은 경우 아래와 같이 답변하면 됩니다.
    다음 형식으로 조인 조건을 추출해주세요 (조인이 없으면 "-"이라고 답변)
        
    customer_id = customer_id        
    cust_name = cust_name

* 주의사항 : Mybatis XML화일의 내용으로 다이나믹하게 조합이 되는 경우도 있습니다.
            조인 조건만 잘 추출해 내주세요.
            
{dynamicQuery}

    상기와 같은 경우 PRODUCT_ID = productId에서 productId는 변수입니다.  
    다른 테이블에 컬럼이 아니므로 조인조건이 아닙니다.
...
아래 SQL에서 두개 이상 테이블이 있는 경우 조인조건만 추출해주세요.
아니면 '-'를 답변합니다.
...
* 추출된 조인 조건:"""
            ...
            response = self._chat_with_debug(client, [{"role": "user", "content": prompt}], max_tokens=3000, temperature=0.3)
            response_text = response.strip() if isinstance(response, str) else str(response).strip()
        ...
        try:
            ...
            for table in tables:
                ...
                for sql in related_sql:                    
                    ...
                    max_len_sql = 10000
                    context += f"- {sql.mapper_ns}.{sql.stmt_id}: {sql_text[:max_len_sql]}...\n"
            ...
            handle_critical_error(logger, "테이블 코멘트 처리 실패", e)
        finally:
            session.close()
...
def generate_source_specification_md(config: Dict[str, Any], output_path: str):
    ...
    try:
        # Get all files (LLM 요약이 없어도 포함)
        files = session.query(File).order_by(File.path).all()
        ...
        for group_name, group_files in file_groups.items():
            ...
            for file in group_files:
                ...
                if file.llm_summary:
                    ...
                    # LLM 요약이 없으면 기본 정보로 설명 생성
                    file_name = Path(display_path).name if display_path else "Unknown"
                    if file_name.endswith('.java'):
                        md_content.append(f"**파일 설명:** {file_name}은 Java 클래스 파일입니다.\n")
                    elif file_name.endswith('.jsp'):
                        md_content.append(f"**파일 설명:** {file_name}은 JSP 뷰 파일입니다.\n")
                    elif file_name.endswith('.xml'):
                        md_content.append(f"**파일 설명:** {file_name}은 XML 설정 파일입니다.\n")
                    else:
                        md_content.append(f"**파일 설명:** {file_name} 파일입니다.\n")
...
def generate_table_specification_md(config: Dict[str, Any], output_path: str):
    ...
    try:
        ...
        for table in tables:
            ...
            for sql in related_sql:
                sql_text = getattr(sql, 'sql_content', '') or getattr(sql, 'normalized_fingerprint', '') or 'No SQL content'
                context += f"- {sql.mapper_ns}.{sql.stmt_id}: {sql_text[:500]}...\n"
