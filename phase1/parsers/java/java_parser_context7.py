...
class JavaParserContext7(BaseParser):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # pregex_patterns 속성이 없어서 _extract_classes에서 오류 발생하는 문제 해결
    ...
    def _build_pregex_patterns(self):
        ...
        method_signature = Group(
            # 어노테이션 (선택적) - 정규식으로 처리
            Optional('@' + AnyLetter() + Optional(AnyLetter() | AnyDigit() | '_') + Optional('(' + OneOrMore(AnyLetter() | AnyDigit() | AnyWhitespace() | '"' | "'" | '(' | ')' | '=' | ',' | '{' | '}') + ')')) +
    ...
    def _extract_classes(self, content: str) -> List[Dict[str, Any]]:
        ...
        if PREGEX_AVAILABLE and 'class_declaration' in self.pregex_patterns:
            # PRegEx 사용
